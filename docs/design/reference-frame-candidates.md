...
generation feedback. Phase 5-1 adds the transient one-rectangle ROI workspace;
real-browser and NVR validation of the shell remains pending user-supplied
observations.**
...
Phase 5-1 consumes the selected frontend candidate through a separate transient
ROI workspace. One rectangle is drawn with unified mouse/touch/pen Pointer
Events; only one active pointer is accepted, the interaction surface alone has
`touch-action: none`, and source-pixel coordinates are recalculated for
responsive display sizes. Candidate changes, result replacement, and image failure
clear draft and committed ROI state. Pointer interruption clears the draft and
active pointer while preserving a prior committed ROI. Persistence, confirmation, movement,
resize handles, keyboard editing, and manifest storage remain deferred. Editing
an applied input marks it dirty and prevents generation until the user applies
it again; editing does not clear already rendered candidates.
