...
| `sample-recording` | Collect generic, timestamped frames from a long NVR recording range at a chosen cadence. | Planned: `--channel`, `--start`, `--duration`, `--interval`, `--chunk-duration`, `--output-dir`, and an explicit source timezone. | Planned durable frame package and structured manifest; no semantic result. | Planned NVR-only workflow. | No. | Recorded NVR data. | Planned; no command or implementation exists. | [Recording Sampling](recording-sampling.md) |
