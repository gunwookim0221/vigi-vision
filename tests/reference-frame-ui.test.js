...
const { candidate, candidateSet, createHarness, deferred, findElement, headingTexts, submit, textOf } = require("./reference-frame-ui-harness.js");
...
test("renders successful thumbnails in API order and exposes backend timing", async () => {
  ...
  assert.match(textOf(harness.results.children[0].children[1]), /Absolute source time/);
